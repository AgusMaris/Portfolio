...
export function ExperienceTabs({ dict }: Props) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 anim-scroll-fade-delay-1">
      ...
      {/* Content */}
      <div className="flex-1 min-h-[300px] grid">
        {entries.map((exp, index) => (
          <div
            key={exp.company}
            className={cn(
              "space-y-4 col-start-1 row-start-1 transition-all ease-out",
              activeTab === index
                ? "opacity-100 translate-y-0 z-10 duration-500 delay-200"
                : "opacity-0 translate-y-4 pointer-events-none -z-10 duration-200"
            )}
